...
dict_markers = {'U': [+2.0, 'o', 'k', 'U + 2.0'], 'B': [+1.0, 'D', 'b', 'B + 1.0'], 'V': [+0.5, 's', 'g', 'V + 0.5'],
...
#         spline2 = CubicSpline(series.index.values.tolist(), series.values.tolist(), bc_type='natural', extrapolate=True)
        spline = Rbf(series.index.values.tolist(), series.values.tolist())

        if jd - date_explosion > 99:
        ...
        if plot:
            ...
            ax.plot(wave_data, flux_data, 'r-', label='Rbf Fit')
#             ax.plot(wave_data, spline2(wave_data), 'r-', label='CubicSpline Fit')
            ax.legend()
            ax.grid()
            plt.show()
            plt.close(fig)
...
output_df = output_df[~((output_df['Date'] == '2016-12-29') & (output_df['FILTER'] == 'U'))]
