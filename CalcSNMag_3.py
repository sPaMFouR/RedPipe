...
for index, file_name in enumerate(list_instrmag):
    ...
    for column in truemag_df:
        if column in obsmag_df:
            tempmag = reject_series(obsmag_df[column] - truemag_df[column])
            indexes = tempmag.index.values
            temperr = add_series([obserr_df[column].loc[indexes], trueerr_df[column].loc[indexes]], err=True)
