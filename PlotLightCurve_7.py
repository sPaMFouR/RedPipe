...
temprad_df = fbolm_df[fbolm_df['Phase'] < 150].copy()
...
xguess[0:] = [8, 39, 92, 124]

myPWLF.fit_with_breaks([np.min(x)] + [8, 39, 92, 124] + [np.max(x)])
...
ax_vabs.plot(vabs_df['Phase'], vabs_df['AbsMag'], color='k', markersize=7, markerfacecolor='grey', markeredgewidth=1,
...
ax_vabs.plot(vabs_df['Phase'], vabs_df['AbsMag'], color='k', markersize=3, markerfacecolor='None', markeredgewidth=1,
             marker='o', ls='-', lw=0.8, alpha=0.8, label='_nolegend_')

nebarr = np.arange(phase_nebstart + 50, phase_nebstart + 250, 1)
ax_vabs.plot(nebarr, 0.0098 * nebarr - 17, ls='--', lw=1, c='k')


ax_vabs.axvline(0, ls='--', c='k', lw=1)
ax_vabs.axvline(90, ls='--', c='k', lw=1)
ax_vabs.text(20, -12.6, s='OPTd~{0:.0f} d'.format(90))
ax_vabs.annotate(s='', xy=(0, -12.5), xytext=(90, -12.5), arrowprops=dict(arrowstyle='<->'))
ax_vabs.text(phase_nebstart + 60, -15.4, s=r'$\rm ^{56}Co \rightarrow\ ^{56}Fe$', fontsize=12, rotation=-32)
        ...
        if not temp_df.empty:
            ...
            if name == '2005cs':
                ...
                ax_bol.plot(temp_df['Phase'], temp_df['Lum'], ls=':', markersize=6, c=sndata_df.loc[name, 'Color'],
                            alpha=0.8, marker=sndata_df.loc[name, 'Marker'], label=name + ' + 0.8')
            else:
                ax_bol.plot(temp_df['Phase'], temp_df['Lum'], ls=':', markersize=6, c=sndata_df.loc[name, 'Color'],
...
ax_bol.plot(xarr, fit, ls='-', lw=2, c='blue', label='PWLF')
...
ax_bol.text(xguess[0] - 13, 40.1, 'Rising Phase', color='goldenrod', fontsize=11, rotation=90)
ax_bol.text(xguess[1] - 21, 40.83, 'Early Plateau Phase [$s_1$={:.2f} dex]'.format(s1), color='tomato', fontsize=11,
            rotation=90)
ax_bol.text(xguess[2] - 33, 40.81, 'Late Plateau Phase [$s_2$={:.2f} dex]'.format(s2), color='tomato', fontsize=11,
            rotation=90)
ax_bol.text(xguess[3] - 20, 40.23, 'Transition Phase', color='goldenrod', fontsize=11, rotation=90)
ax_bol.text(xguess[3] + 30, 39.74, 'Nebular Phase [$s_3$={:.2f} dex]'.format(s3), color='tomato', fontsize=11)

# ax_bol.axvspan(-10, xguess[0], color='grey', alpha=0.3)
# ax_bol.axvspan(xguess[0], xguess[1], color='grey', alpha=0.3)
# ax_bol.axvspan(xguess[1], xguess[2], color='darkorange', alpha=0.3)
# ax_bol.axvspan(xguess[2], xguess[3], color='skyblue', alpha=0.3)
# ax_bol.axvspan(xguess[3], max_epoch + 20, color='pink', alpha=0.3)
...
ax_bol.set_ylabel(r'Pseudo-Bolometric Luminosity [$\rm erg\ s^{-1}$]', fontsize=16)
