...
def calc_objlum(flux, name=name_SN):
    if name != name_SN:
        dval = data.loc[name, 'D']
        derr = data.loc[name, 'DErr']
    else:
        dval = dist_val
        derr = dist_err
...
def calc_magflux(mag, err, band, name=name_SN):
    ...
    if name != name_SN:
        dval = data.loc[name, 'D']
        derr = data.loc[name, 'DErr']
        ebvmag = data.loc[name, 'EBV']
        ebverr = data.loc[name, 'EBVErr']
    else:
        dval = dist_val
        derr = dist_err
        ebvmag = EBV_mag
...
def coltorow_df(name, data_df):
    ...
    for index, band in input_df['FILTER'].items():
        magflux = calc_magflux(mag=input_df.loc[index, 'FMAG'], err=input_df.loc[index, 'FERR'], band=band, name=name)
...
def calc_boldf(name, input_df, plot=False):
    ...
    for jd in input_df.columns.values:
        ...
        dict_flux[jd]['Lum'] = calc_objlum(netflux, name=name)[0]
        dict_flux[jd]['LumErr'] = calc_objlum(netflux, name=name)[1]
...
outputuv_df['JD'] = (outputuv_df['JD'] + epoch).round(1)
outputuv_df['Date'] = outputuv_df['JD'].apply(jd_to_cald)
outputuv_df['Phase'] = (outputuv_df['JD'] - date_explosion).round(1)

for index, band in outputuv_df['FILTER'].iteritems():
    data_magflux = calc_magflux(mag=outputuv_df.loc[index, 'FMAG'], err=outputuv_df.loc[index, 'FERR'], band=band)
    outputuv_df.loc[index, 'AbsMag'] = data_magflux[0]
    outputuv_df.loc[index, 'AbsErr'] = data_magflux[1]
    outputuv_df.loc[index, 'Flux'] = data_magflux[2]
    outputuv_df.loc[index, 'FluxErr'] = data_magflux[3]
    outputuv_df.loc[index, 'ALambda'] = float(fmt_flt.format(filter_df.loc[band, 'RLambda'] * EBV_mag))

outputnet_df = pd.concat([outputopt_df, outputuv_df])
outputuv_df = outputuv_df.set_index('JD').dropna(axis=0, how='any')
outputuv_df['Mag'] = outputuv_df['FMAG'].apply(lambda x: "{:.2f}".format(x)) + r'$\pm$' +
outputuv_df['FERR'].apply(lambda x: "{:.2f}".format(x))
...
bolm_df, fbolm_df = organise_dataframe(outputnet_df)
# ------------------------------------------------------------------------------------------------------------------- #
...
tabopt_df = unorgframe_to_orgframe(data_df, column='Mag')

tabopt_df['Phase'] = tabopt_df['Date'].apply(lambda x: data_df.loc[data_df.index == x, 'Phase'].iloc[0])
tabopt_df['JD'] = tabopt_df['Phase'] + date_explosion - JD_offset
...
# ------------------------------------------------------------------------------------------------------------------- #
tabuv_df = unorgframe_to_orgframe(outputuv_df, column='Mag')
...
tabuv_df = tabuv_df.rename(columns={'Phase': 'Phase$^*$', 'uvw2': '$uvw2$', 'uvm2': '$uvm2$', 'uvw1': '$uvw1$',
                                    'uvu': '$uvu$', 'uvv': '$uvv$'})
tabuv_df = tabuv_df.replace(np.nan, '---', regex=True)
...
for file_name in list_files:
    ...
    if name in ['1999em', '2004et', '1987A', '2012ec', '2013ab']:
        ...
        if not temp_df.empty:
            if name != '1987A':
                ax_vabs.plot(temp_df['Phase'], temp_df['AbsMag'], linestyle=':', markersize=6,
                             marker=data.loc[name, 'Marker'], c=data.loc[name, 'Color'], label=name)
            else:
                ax_vabs.plot(temp_df['Phase'].iloc[::2], temp_df['AbsMag'].iloc[::2], linestyle=':', markersize=6,
                             marker=data.loc[name, 'Marker'], c=data.loc[name, 'Color'], label=name)
...
#                             marker=data.loc[name, 'Marker'],c=data.loc[name, 'Color'], label=name)
            if name != '1987A':
                ...
                ax_bol.semilogy(temp_df['Phase'].iloc[::2], temp_df['Lum'].iloc[::2], linestyle=':', markersize=6,
                                marker=data.loc[name, 'Marker'], c=data.loc[name, 'Color'], label=name)

ax_bol.semilogy(fbolm_df['Phase'], fbolm_df['Lum'], linestyle='-', color='k', marker='o', markersize=6, label=name_SN)
