...
# ------------------------------------------------------------------------------------------------------------------- #
FILE_AIRMASS = '/home/avinash/PyCharmProjects/Reduction_Pipeline/airmas.asc'
...
# ------------------------------------------------------------------------------------------------------------------- #
lower_limit_gr7 = 3750
upper_limit_gr7 = 7300
lower_limit_gr8 = 7200
upper_limit_gr8 = 9150
# ------------------------------------------------------------------------------------------------------------------- #
...
def bias_subtract(text_list_tbs, master_bias, prefix_string='bs_'):
    """
    ...
    Args:
        ...
        master_bias   : Name of the master bias file
        prefix_string : Prefix to distinguish the aligned FITS file from the original FITS file
    Returns:
        None
    """
...
def imcopy_clip(text_list_clip, clip_section, prefix_string='c'):
    """
...
def crmedian(text_list_cosmic, clip_section, prefix_string='c'):
    """
    ...
    imcopy_clip(text_list_clip=str(text_list_cosmic), clip_section=str(clip_section), prefix_string='')
...
def cosmicray_check(text_list_cosmic, prefix_string='cr_'):
    """
    ...
    for file_name in list_cosmic:
        output_file_name = str(prefix_string) + str(file_name)[3:]
        remove_file(str(output_file_name))
...
def dispcor(text_list_wcalib, prefix_string='w'):
    """
...
def standard(file_name, prefix_string='std_'):
    """
    ...
    output_file_name = str(prefix_string) + str(star_name) + str(grism_suffix)
    remove_file(str(output_file_name))
...
def sensfunc(file_name, prefix_string='sens_'):
    """
...
def calibrate(file_name, prefix_string='f'):
    ...
    if grism_name == '4Grism7':
        ...
        print ("Error : Grism Keyword Has A Different Value - {0}".format(str(grism_name)))
...
def scopy_sens(file_name):
    """
    Copies the clipped version (based on wavelength) of the file 'file_name' onto the same file.
    To be done on wavelength calibrated spectras.
    Args:
        file_name       : Name of the FITS file to be copied
    Returns:
        None
    """
    file_header = fits.getheader(file_name)
...
lower_limit_gr7 = 3750
upper_limit_gr7 = 6200
lower_limit_gr8 = 6100
upper_limit_gr8 = 9150


def scopy_fcb(file_name, prefix_string='c'):
    """
    ...
    if grism_name == '4Grism7':
        lower_limit = lower_limit_gr7
        upper_limit = upper_limit_gr7
    elif grism_name == '3Grism8':
        lower_limit = lower_limit_gr8
        upper_limit = upper_limit_gr8
    else:
        print ("Error : Grism Keyword Has A Different Value - {0}".format(str(grism_name)))
...
def scombine(text_list_comb):
    """
    Combines the spectras in the two grisms (Grism7 & Grism8) based on a common sampling region.
    Args:
        text_list_comb : Text list of spectras to be combined
    Returns:
        None
    """
    list_comb = text_list_to_python_list(str(text_list_comb))
    print(list_comb)
    sampling_region = str(lower_limit_gr8 + 25) + ':' + str(upper_limit_gr7 - 25)
    ...
    if len(list_comb) == 2:
        ...
        print ("Error : Needs Both Grism7 & Grism8 Clipped Flux Calibrated 1-D Spectras To Combine")
...
def cosmicray_removal(text_list_cosmic, clip_section, prefix_string='c'):
    """
    Corrects for cosmic rays in the OBJECT image after clipping based on the string 'clip_section'
    Args:
    ...
    Returns:
        None
    """
    imcopy_clip(text_list_clip=str(text_list_cosmic), clip_section=clip_section, prefix_string='')
    list_cosmic = text_list_to_python_list(str(text_list_cosmic))
...
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Groups BIAS Subtracted Spectras Of Objects+Standards & Lamps
# # ------------------------------------------------------------------------------------------------------------------- #
...
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Appends Header Keyword 'REFSPEC1' With Desired Value For Objects & Standards
# # ------------------------------------------------------------------------------------------------------------------- #
...
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Calculates AIRMASS Of Wavelength Calibrated 1-D Spectra Of Objects & Standards
# # ------------------------------------------------------------------------------------------------------------------- #
# list_wcbs = group_similar_files("list_wcbs", common_text="wcbs_*.fits")
# calculate_airmass("list_wcbs")
# # ------------------------------------------------------------------------------------------------------------------- #
#
#
# # ------------------------------------------------------------------------------------------------------------------- #
...
#
# for file_name in list_wcbs:
#     scopy_sens(file_name)
#
...
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Performs Flux Calibration On Object Spectras Using CCD's Sensitivity Function
# # ------------------------------------------------------------------------------------------------------------------- #
# list_wcbs_object = group_similar_files("", common_text="wcbs_*ASASSN*")
#
# for file_name in list_wcbs_object:
#     calibrate(file_name)
# # ------------------------------------------------------------------------------------------------------------------- #
#
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Performs Clipping On Flux Calibrated Object Spectras
# # ------------------------------------------------------------------------------------------------------------------- #
# list_fwcbs = group_similar_files("", common_text="fwcbs_*gr*")
#
# for file_name in list_fwcbs:
#     scopy_fcb(file_name)
# # ------------------------------------------------------------------------------------------------------------------- #
#
#
# # ------------------------------------------------------------------------------------------------------------------- #
# # Performs Combine Operation On Clipped Flux Calibrated Object Spectras (If Both Grism7 & Grism8 Data Exist)
# # ------------------------------------------------------------------------------------------------------------------- #
# group_similar_files("list_cfwcbs", common_text="cfwcbs_*gr*")
# scombine("list_cfwcbs")
# # ------------------------------------------------------------------------------------------------------------------- #
...
# Performs Combine Operation On Clipped Flux Calibrated Object Spectras (If Both Grism7 & Grism8 Data Exist)
# ------------------------------------------------------------------------------------------------------------------- #
group_similar_files("list_fwcbs", common_text="fwcbs_*gr*")
scombine("list_fwcbs")
# ------------------------------------------------------------------------------------------------------------------- #
