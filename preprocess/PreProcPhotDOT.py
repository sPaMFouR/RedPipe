...
# Global Variables
# ------------------------------------------------------------------------------------------------------------------- #
filters_headname = ['7BesU', '6BesB', '5BesV', '4BesR', '3BesI', 'SDSSg']
...
DIR_PHOT = DIR_SN + "DOT/Photometry/"
# ------------------------------------------------------------------------------------------------------------------- #
...
def flat_combine(ctext):
    ...
    if check_fileempty(textlist_nbsflat):
        display_text("ERROR: No FLAT Images Found For Band {0} In Directory {1}".format(band, os.getcwd()))
    else:
...
for band in filters:
    group_similar_files('list_' + band, '*-' + band + '*.fits')
...
for index, band in enumerate(filters):
    hedit('list_' + band, FILTER_keyword, filters_headname[index])
# ------------------------------------------------------------------------------------------------------------------- #
...
for band in filters:
    flat_combine(ctext='nbs_flat-' + band + '*.fits')

for band in filters:
    flat_correction(ctext='bs_*-' + band + '*.fits', master_flat='mflat' + band + '.fits')
# ------------------------------------------------------------------------------------------------------------------- #
...
for file_name in group_similar_files('', common_text='cfbs_{0}*.fits'.format(OBJECT_name)):
    hdulist = fits.open(file_name, ext=0)
    hdulist.verify('silentfix')
    header = hdulist[0].header
    date = header[DATE_keyword]
    date_obs = date.split('"T"')[0]
