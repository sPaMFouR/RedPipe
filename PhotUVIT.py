...
# ------------------------------------------------------------------------------------------------------------------- #
list_files = group_similar_files('', common_text=ctext, exceptions='Comb')
...
for index, file_name in enumerate(list_files):
    ...
    aper_phot(file_name, list_coord[index], phot_radius=aperture_values, annulus=20, dannulus=2)
# ------------------------------------------------------------------------------------------------------------------- #
...
output_cols = ['PHASE', 'EXPTIME', 'COUNTS', 'MAG', 'ERR']

list_magwithgal = group_similar_files('', common_text='*.mag.1')
list_magwithoutgal = group_similar_files('', common_text='*.mag.2')
...
dict_magwithoutgal = {}
for file_name in list_magwithoutgal:
    header = fits.getheader(file_name[:-7], ext=0)
    exptime = float(header[EXPTIME_keyword])
    ...
    obsepoch = header[DATE_keyword] + 'T' + header[TIME_keyword][:-6]
    dict_magwithoutgal[obsepoch] = {}
    dict_magwithoutgal[obsepoch]['PHASE'] = round((header[TSTART_keyword] + header[TSTOP_keyword]) / 2., 3)
    dict_magwithoutgal[obsepoch]['EXPTIME'] = round(exptime, 3)
    dict_magwithoutgal[obsepoch]['COUNTS'] = round(counts, 7)
    dict_magwithoutgal[obsepoch]['MAG'] = round(-2.5 * np.log10(counts) + zpmag, 3)
    dict_magwithoutgal[obsepoch]['ERR'] = round(((1.087 / ((counts * exptime) ** 0.5)) ** 2 + zperr ** 2) ** 0.5, 3)

data_magwithoutgal = pd.DataFrame(dict_magwithoutgal).T
data_magwithoutgal.index.name = 'OBSTIME'
data_magwithoutgal = data_magwithoutgal[output_cols]
data_magwithoutgal.to_csv('OUTPUT_MagWithoutGal', sep=' ', index=True, header=True)
...
# ------------------------------------------------------------------------------------------------------------------- #
data_magwithgal['HOUR'] = data_magwithgal['PHASE'].apply(lambda x: (x - data_magwithgal['PHASE'].min()) / 3600)

fig = plt.figure(figsize=(12, 8))
ax = fig.add_subplot(111)

ax.errorbar(data_magwithgal['HOUR'], data_magwithgal['MAG'], yerr=data_magwithgal['ERR'], marker='*', color='k',
            markersize=12, linestyle='--', capthick=1, capsize=5, elinewidth=1)
...
fig.savefig('PLOT_UVIT-F172M.eps', format='eps', dpi=1000, bbox_inches='tight')
# plt.show()
plt.close(fig)
# ------------------------------------------------------------------------------------------------------------------- #
