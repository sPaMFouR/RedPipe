...
for file_name in list_halpha:
    ...
    if name in ['2012aw', '2013ab', '2013ej', '2004et', '1999em']:
        ...
        ax2.plot(temp_df['Phase'], temp_df['Vel'] / 1e3, ls=':', lw=1.2, marker=dict_snmark[name][0], ms=10,
                 c='k', markerfacecolor=dict_snmark[name][1], alpha=0.6, label=name)
...
for file_name in list_phot:
    ...
    if name in ['2012aw', '2013ab', '2013ej', '2004et', '1999em']:
        temp_df = pd.read_csv(file_name, sep='\s+', header=None, comment='#', names=['Phase', 'Vel'])
        temp_df = temp_df[temp_df['Phase'] < plot_epoch]
        ax3.plot(temp_df['Phase'], temp_df['Vel'] / 1e3, ls=':', lw=1.2, marker=dict_snmark[name][0], ms=10,
                 c='k', markerfacecolor=dict_snmark[name][1], alpha=0.6, label=name)

ax2.axvline(8, ls='--', lw=0.8, c='k')
ax2.axvline(25, ls='--', lw=0.8, c='k')
ax2.axhline(13, ls='--', lw=0.8, c='navy')
ax2.axhline(11, ls='--', lw=0.8, c='red')
ax2.axvspan(8, 25, color='grey', alpha=0.2)

ax2.plot(vel_df.index.values, vel_df['6563'] / 1e3, marker='*', ms=15, ls='', c='k', alpha=0.8, label=name_SN)
ax3.plot(vel_df.index.values, vel_df['5169'] / 1e3, marker='*', ms=15, ls='', c='k', alpha=0.8, label=name_SN)
...
ax2.text(15, 8, s='Ejecta-CSM Interaction', color='k', alpha=0.7, rotation=90, fontsize=12)
ax2.text(50, 14.7, s=r'$\rm Power\ Law\ Exponent: {0:.3f}\pm{1:.3f}$'.format(opt2[1], err2[1]), fontsize=14)
ax3.text(15, 10.4, s=r'$\rm Power\ Law\ Exponent: {0:.3f}\pm{1:.3f}$'.format(opt3[1], err3[1]), fontsize=14)
