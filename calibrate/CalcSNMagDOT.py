...
def calculate_psfmag(alsfile, zpmag_df, zperr_df):
    """
    Calculates true magnitudes of the SN from Tx'dumped ALS & MAG files obtained after photometry.
    Arg
        alsfile     : File containing Tx'dumped magnitudes obtained from ALS files generated by IRAF
        zpmag_df    : Pandas DataFrame containing zero-point magnitudes
        zperr_df    : Pandas DataFrame containing zero-point errors
    Returns:
        identifier  : Identifier used to different files on which photometry is being performed
        psfsn_df    : Pandas DataFrame containing PSF magnitudes and errors of the transient
    """
    star_df = pd.read_csv(file_starscoo, sep='\s+', header=None)
    star_count = len(star_df.index.values)
    identifier = alsfile.split('_')[1]

    psf_df = pd.read_csv(alsfile, sep='\s+', names=list_psfcol)
    ...
    magfile = alsfile[:-4] + 'mag4'
    mag_df = pd.read_csv(magfile, sep='\s+', names=list_magcol)
    ...
    for band in set(psfstars_df['FILTER'].values):
        ...
        psfcor_meanerr[band] = (np.sum([val ** 2 for val in psfcor_errs]) / len(psfcor_errs)) ** 0.5

    psfsn_df['PSFCOR_MEAN'] = psfsn_df['FILTER'].apply(lambda x: psfcor_mean[x])
    psfsn_df['PSFCOR_MEANERR'] = psfsn_df['FILTER'].apply(lambda x: psfcor_meanerr[x])
    psfsn_df['PSFCOR_STD'] = psfsn_df['FILTER'].apply(lambda x: psfcor_stdev[x])
    psfsn_df['ZP_MAG'] = psfsn_df['FILTER'].apply(lambda x: zpmag_df.loc[identifier, x])
    psfsn_df['ZP_ERR'] = psfsn_df['FILTER'].apply(lambda x: zperr_df.loc[identifier, x])
    ...
    psfsn_df.to_csv('OUTPUT_SNMag_' + identifier, sep=' ', index=False)

    return identifier, psfsn_df
    

def calculate_tempmag(magfile, zpmag_df, zperr_df):
    """
    ...
    Returns:
        identifier  : Identifier used to different files on which photometry is being performed
        temp_df     : Pandas DataFrame containing Temmplate subtracted magnitudes and errors of the transient
    """
    temp_df = pd.read_csv(filepath_or_buffer=magfile, sep='\s+', names=list_magcol, index_col=0)
    temp_df['FILTER'] = temp_df['IFILTER'].apply(lambda x: str(x)[-1])
    temp_df = temp_df.replace('INDEF', np.nan)
    temp_df[['MAG_1', 'ERR_1', 'MAG_2', 'ERR_2']] = temp_df[['MAG_1', 'ERR_1', 'MAG_2', 'ERR_2']].astype('float64')
    temp_df = temp_df.sort_index().sort_values(by='FILTER', kind='mergesort')

    star_count = len(set(temp_df.index.values))
    mag4file = magfile[:-4] + 'mag4'
    identifier = magfile.split('_')[1]
    
    mag_df = pd.read_csv(filepath_or_buffer=mag4file, sep='\s+', names=list_magcol, index_col=0)
    ...
    temp_df['ZP_MAG'] = temp_df['FILTER'].apply(lambda x: zpmag_df.loc[identifier, x])
    temp_df['ZP_ERR'] = temp_df['FILTER'].apply(lambda x: zperr_df.loc[identifier, x])
    ...
    temp_df.to_csv('OUTPUT_SNMagTemp_' + identifier, sep=' ', index=False)

    return identifier, temp_df
...
truemag_df = pd.read_csv(file_mag, sep='\s+', index_col=0)
trueerr_df = pd.read_csv(file_err, sep='\s+', index_col=0)
...
for index, file_name in enumerate(list_instrmag):
    ...
    obsmag_df = pd.read_csv(file_name, sep='\s+', index_col=0)
    obserr_df = pd.read_csv(list_instrerr[index], sep='\s+', index_col=0)
    ...
    dict_zpmag[identifier] = {}
    dict_zperr[identifier] = {}

    for band in truemag_df:
        if band in obsmag_df:
            tempmag = reject_series(obsmag_df[band] - truemag_df[band])
        ...
        dict_zpmag[identifier][band] = round(zpmag, int(precision))
        dict_zperr[identifier][band] = round(zperr, int(precision))
...
for file_name in list_fits:
    identifier = file_name.split('_')[1] + '-' + file_name.split('-')[-1].split('.')[0]
    JD, _, ifilter = calculate_jd(file_name)
    if identifier not in dict_JD:
        dict_JD[identifier] = {}
    dict_JD[identifier][ifilter] = round(float(JD), int(precision))
...
for idx, file_name in enumerate(list_psfmag):
    identifier, psf_df = calculate_psfmag(file_name, zpmag_df, zperr_df)
    indexes = psf_df.index.values
    file_df = pd.Series(name='FileName', index=indexes, data=[identifier] * len(indexes))
    finalmag_df = pd.concat([finalmag_df,
                             pd.concat([psf_df, file_df, JD_df.loc[:, identifier].rename('JD')], axis=1, sort=True)])

finalmag_df.index.name = 'FILTER'
finalmag_df = finalmag_df.dropna().reset_index().set_index('JD', drop=True)
#finalmag_df = finalmag_df.sort_values(by='FILTER').sort_index(kind='mergesort')
finalmag_df = finalmag_df[['FileName', 'FILTER', 'FMAG', 'FERR']]
finalmag_df.to_csv('OUTPUT_FinalSNMag', sep=' ', index=True)
...
for file_name in list_tempmag:
    identifier, mag_df = calculate_tempmag(file_name, zpmag_df, zperr_df)
    tempmag_df = pd.concat([tempmag_df, pd.concat([mag_df, JD_df.loc[:, identifier].rename('JD')], axis=1, sort=True)])
